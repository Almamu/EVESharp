...
decompiled_store_path = decompiled/
compiled_store_path = compiled/
...
def process_func(code_q, result_q, decompiled_store_path, compiled_store_path, lock):
    ...
    try:
        ...
        while 1:
            ...
            try:
                ...
                # generate the compiled filepath
                compiled_filename = os.path.join(compiled_store_path, filename + 'c')
                compiled_filename = os.path.abspath(compiled_filename)
                # prepend our store_path
                filename = os.path.join(decompiled_store_path, filename)
...
if __name__ == '__main__':
    ...
    decompiled_store_path = os.path.join(config.get('main', 'decompiled_store_path'),
      'eve-%s.%s' % (eveconfig.get('main', 'version'), eveconfig.get('main', 'build')))
    decompiled_store_path = os.path.abspath(decompiled_store_path)

    compiled_store_path = os.path.join(config.get('main', 'compiled_store_path'),
      'eve-%s.%s' % (eveconfig.get('main', 'version'), eveconfig.get('main', 'build')))
