...
namespace Node
{
    public class CacheStorage : DatabaseAccessor
    {
        ...
        public static string[] LoginCacheQueries =
        {
            ...
            "SELECT itemID AS ownerID, itemName AS ownerName, typeID FROM entity LEFT JOIN invTypes USING (typeID) LEFT JOIN invGroups USING (groupID) WHERE categoryID = 1 AND itemID < 90000000",
            ...
            "SELECT itemID AS locationID, itemName as locationName, x, y, z FROM entity LEFT JOIN invTypes USING (typeID) LEFT JOIN invGroups USING (groupID) WHERE (categoryID = 2 OR categoryID = 3) AND itemID < 90000000",
