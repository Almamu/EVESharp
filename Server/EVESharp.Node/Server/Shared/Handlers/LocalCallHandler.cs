...
public class LocalCallHandler
{
    ...
    public LocalCallHandler(IMachoNet machoNet, MessageProcessor processor, ILogger logger, ServiceManager serviceManager, BoundServiceManager boundServiceManager)
    {
    ...
    {
        this.MachoNet.Log.Verbose($"Calling {service}::{method}");

        return this.ServiceManager.ServiceCall(service, method, call);
    }
    ...
    private PyDataType HandleBoundCallReq(PyTuple information, string method, CallInformation call)
    {
        ...
        return this.BoundServiceManager.ServiceCall(boundID, method, call);
    }
    ...
    public void HandleCallReq(MachoMessage machoMessage)
    {
        ...
        else if (packet.Destination is PyAddressNode node)
        {
            ...
            if (node.NodeID != this.MachoNet.NodeID)
        ...
        if (machoMessage.Packet.OutOfBounds is not null && machoMessage.Packet.OutOfBounds.TryGetValue("Session", out PyDictionary dictionary) == true)
        ...
        CallInformation callInformation = new CallInformation
        {
            ...
            MachoNet = this.MachoNet,
            Session = session,
            ResultOutOfBounds = new PyDictionary<PyString, PyDataType>(),
            ServiceManager = this.ServiceManager,
            BoundServiceManager = this.BoundServiceManager
        };
    ...
    private void SendProvisionalResponse(CallInformation answerTo, ProvisionalResponse response)
    {
        ...
        this.MachoNet.QueueOutputPacket(null, result);
    }
