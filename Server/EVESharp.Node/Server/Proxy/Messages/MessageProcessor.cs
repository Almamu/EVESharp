...
public class MessageProcessor : Shared.Messages.MessageProcessor
{
    ...
    public MessageProcessor(IMachoNet machoNet, ILogger logger, ServiceManager serviceManager, BoundServiceManager boundServiceManager, SessionManager sessionManager) :
    ...
    {
        if ((machoMessage.Packet.Destination is PyAddressNode node && node.NodeID == this.MachoNet.NodeID) ||
        ...
        if (machoMessage.Transport is MachoClientTransport &&
            machoMessage.Packet.OutOfBounds.TryGetValue("OID-", out PyDictionary deleted) == true)
        {
            foreach ((PyString guid, PyInteger refID) in deleted.GetEnumerable<PyString, PyInteger>())
            {
                ...
                this.BoundServiceManager.ClientHasReleasedThisObject(boundID, machoMessage.Transport.Session);
            }
            ...
            foreach((int boundID, long nodeID) in machoMessage.Transport.Session.BoundObjects)
                if (machoMessage.Transport.Session.NodesOfInterest.Contains(nodeID) == false)
                 ...
                 machoMessage.Packet.OutOfBounds.TryGetValue("OID+", out PyList added) == true)
        {
            ...
            if (this.MachoNet.TransportManager.ClientTransports.TryGetValue(destination.ClientID, out MachoClientTransport transport) == false)
    ...
    private void HandleAnyPacket(MachoMessage machoMessage)
    {
        switch (machoMessage.Packet.Type)
        {
            ...
            case PyPacket.PacketType.PING_REQ:
                ...
                this.LocalPingHandler.HandlePingReq(machoMessage);
                break;
